    ...
    <div className="space-y-6">
      ...
      <BottomSheet open={showCryptoModal} onClose={() => setShowCryptoModal(false)} title="Tambah Kepemilikan Crypto">
        <form onSubmit={saveCrypto} className="space-y-4">
          <div><label className="block text-xs font-medium text-foreground mb-1.5">Coin (Top 100)</label><select required value={cryptoForm.coin_id} onChange={e => setCryptoForm({ ...cryptoForm, coin_id: e.target.value })} className="input-field"><option value="">Pilih coin</option>{cryptoCoins.map(c => <option key={c.id} value={c.id}>{c.name} ({c.symbol.toUpperCase()})</option>)}</select></div>
